...
LABEL_PM_START:
	...
	; change ELF kernel into a binary kernel
	call	InitKernel
...
DispMemInfo:
	...
	mov	esi, MemChkBuf		; a buffer of 256 Bytes
...
; 启动分页机制 --------------------------------------------------------------
SetupPaging:
	; 根据内存大小计算应初始化多少PDE以及多少页表.pde: page directory entry
	xor	edx, edx
	mov	eax, [dwMemSize]
	mov	ebx, 400000h	; 400000h = 4M = 4096 * 1024, 一个页表对应的内存大小
	div	ebx		; edx&eax / ebx = eax ... edx
	mov	ecx, eax	; 此时 ecx 为页表的个数，也即 PDE 应该的个数. the number of items included in PDE
	test	edx, edx
	jz	.no_remainder
...
.1:
	...
	mul	ebx			; total num of PTE is stored in EDX-EAX, but edx must be 0, coz eax means 4G PTE, that is 16TB RAM
	mov	ecx, eax		; PTE个数 = 页表个数 * 1024
	mov	edi, PageTblBase	; 此段首地址为 PageTblBase
	xor	eax, eax
	mov	eax, PG_P  | PG_USU | PG_RWW
.2:
	stosd				; es has been initialized before doing .1 loop
	add	eax, 4096		; 每一页指向 4K 的空间. value in pte ranges from 0 to the total size( properties not included)
	loop	.2

	mov	eax, PageDirBase	; store PageDirBase to cr3, so that cpu can find our page directory
	mov	cr3, eax
	mov	eax, cr0
	or	eax, 80000000h		; turn on the switch, so that cpu can work in paging mode
	mov	cr0, eax
	jmp	short .3
.3:
	nop
...
_dwMemSize:			dd	0	; store the size of RAM after the 'DispRAMInfo' is called
